**Tehtävä x)**
- 4.12.1 Ohjelmointi kielissä taskit mitä toistetaan usein ovat funktioita. Puppet sen sisaan määrittää itse konseptinsa ja tarkoituksensa.
- 4.12.2 Karvinen toteaa tekstissään, että jotkut ohjelmointi kaavat kuten package-file-service kaava on yleinen konfiguraatio ja CM työkalut eivät välttämättä kompleksisuudestaan huolimatta ole todella työläitä.(Karvinen 2024, 4) Omana huomiona olen huomannut tiettyjen kaavojen toistumisen myös itse kurssin aikana mistä tekstissä mainitaan.
- 4.12.3 Käytin alla olevissa tehtävissä Jinja2:sta, koska se käyttää loogista syntäksia kuten muuttujat. Mikä on linjassa tekstissä olevan väitteen kanssa "Dialect of existing language faster to learn".(Karvinen 2024, 4)
  
**Tehtävä a)** (Caddy s.a.)
- Aluski asensin Caddyn ja se tapahtui komennoilla sudo apt update ja sudo apt install caddy (muuta ei tarvita koska teen vain localhostina)
- sitten käytiin kattomassa että kaikki tarvittava on caddyfilessä sudo nano /etc/caddy/Caddyfile ja laitettiin sinne <pre>localhost {
    root * /var/www/html
    file_server
} </pre>
...
- Seuraavaksi muokkasin index.html tiedostoa että saan sinne omaa tekstiä. Menin hakemimistoon /var/www/html ja kirjotin index.html (sudo nano index.html) ja sinne pienen tekstin.
...
- eli tässä play bookissa hyvin minimalistiset taskit extrana olisi voinut vielä laittaa että sulkee ja disablee kurssilla aijemmin käytetytyt apache2 ja nginx, koska voisi tulla ongelmia koska käyttävät samaa porttia. Itse olen ne disablannut joten sitä ei tässä localhost versiossa tarvittu
- sen voisi tehdä esim
...
**Tehtävä c)** 
- Aluksi luodaan samaan kansioon missä playbook on Jinja2 tiedosto
...
- Käytetään siksi templatea eikä copya, koska silloin ansible ei vain kopioi tiedostoa palvelimelle vaan lukee sen ja etsii Jinja2-syntaksia
- Kopio konfiguraation loppuun on pistetty notify: reload caddy siksi koska se informoi Handlerssin että sitä tarvitaan ja handlers reloudaa silloin caddyn.
...
- eli kuten kuvasta näämme homma pelittää eli playbookki toimii vaikka caddyn poistaisi, playbook korjaa tilanteen
...
- Karvinen, T. 2024. Configuration Management of Distributed Systems over Unreliable and Hostile Networks. PhD thesis. University of Westminster. Doctor of Philosophy. Luettavissa: https://westminsterresearch.westminster.ac.uk/item/w7vvz/configuration-management-of-distributed-systems-over-unreliable-and-hostile-networks. Luettu: 20.4.2026
